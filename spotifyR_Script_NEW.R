...
# Data splitting #
# The end-argument in the window-function indicates the 10th of March (day 70 of
# the year 2020)

spotifyR_TS_orig <- window(spotifyR_TS, end = c(2020, 70)) 

# We estimate the median stream counts of 111 days (that is the analyzed
# period of the COVID-19-pandemic of interest); start and end days
# are included.
...
# Are the data stationary? If not: How many times does the data need to be 
# differenced?
...
# ADF: Null hypo: Data have a unit-root. That is, we want reject the null 
# (i.e., p < .05)
...
# year 2020), and June 29th, 2020 (day 181 of the year 2020); this is the 
...
set.seed(1897, sample.kind = "Rounding")
...
for (i in 1:pred_period) {
  ...
  # once one data point is predicted, it will be accordingly substracted.
  
  # Substracting/Removing the respective first data point. 
  y_train <- y_train[-1] 
  
  # Substracting/Removing the respective last data point.  
...
# De-transforming the estimates by taking the "anti-difference" and "anti-logs" 
...
# Extracting the median observation from the training time-series.
mdn_observation <- median(spotifyR_TS_orig)

# Getting the final values by using the median observation as the reference value
# according to which the predicted difference refers (based on the time delay 
# embedded training data). 

knn_de_transformed_estimates <- mdn_observation * knn_exponents_of_estimates
rf_de_transformed_estimates <- mdn_observation * rf_exponents_of_estimates

# Converting them into time series-format

knn_y_pred <- ts(knn_de_transformed_estimates, start = c(2020, 71), 
                 frequency = 365)

rf_y_pred <- ts(rf_de_transformed_estimates, start = c(2020, 71), 
                frequency = 365)
...
# Test (W = 0.9793, p = 0.08218) based on an alpha-level of 5 %, we can
...
spotifyR_tib_star %<>% 
  mutate(rf_E_low_95 = rf_Estimates - (1.96 * SD_star),
         rf_E_high_95 = rf_Estimates + (1.96 * SD_star))
...
spotifyR_tib_star %>%
  ...
  # Prediction Interval  
  geom_ribbon(aes(y = rf_Estimates, ymin = rf_E_low_95, ymax = rf_E_high_95),
